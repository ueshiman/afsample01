...
## 目的
`Tutorial02` の `POST /api/orchestrator/execute` が、`agentsettings.json` の設定に従ってマルチエージェント実行され、結果を `callbackUrl` へ非同期返却することを確認する。
...
`Tutorial02/agentsettings.json` の以下を確認:
...
```powershell
dotnet run --project .\Tutorial02\Tutorial02.csproj
```
...
## 4. 実行モード確認
`Tutorial02/agentsettings.json` の `execution.mode` を切り替えて、各モードで 3. を再実行:
- `sequential`
- `parallel`
